...
**The tree generated by `graph-easy` deviates from the typical tree structure. However, utilizing our debug print_ast function will yield a `.dot` file, which can be converted to an image via the command line using `graphviz.`** 
